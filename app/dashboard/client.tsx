...
import { Dog, Bell, Settings, ChevronRight, TrendingDown, Calendar, Layers, Plus } from 'lucide-react'
...
interface Fynd {
  id: string
  typ: 'besparing' | 'bradskande' | 'uppmarksamhet'
  titel: string
  detalj: string
  action: string
}
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <main style={{ maxWidth: '680px', margin: '0 auto', padding: '24px 20px 80px' }}>
        ...
        <motion.div
          ...
          style={{
            ...
            padding: '28px 24px 24px', marginBottom: '16px',
        ...
        >
          ...
          <div style={{ position: 'absolute', bottom: '-60px', left: '-20px', width: '160px', height: '160px', borderRadius: '50%', background: 'rgba(255,255,255,.04)', pointerEvents: 'none' }} />

          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
            <div>
              <p style={{ fontSize: '12px', fontWeight: 600, color: 'rgba(255,255,255,.6)', letterSpacing: '.05em', textTransform: 'uppercase', marginBottom: '2px' }}>Ditt företag</p>
              <p style={{ fontSize: '17px', fontWeight: 700, color: '#fff' }}>{company}</p>
            </div>
            <MascotBubble
              message={attAtgarda > 0 ? `${attAtgarda} saker att kolla på 👋` : 'Allt ser bra ut! 🐾'}
              size="sm"
            />
          </div>

          <div style={{ marginBottom: '6px' }}>
            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,.65)', fontWeight: 500, marginBottom: '4px' }}>Löpande kostnader / mån</p>
            ...
            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,.55)', marginTop: '2px' }}>= {fmtKr(perAr)} per år</p>
          </div>

          <div style={{ display: 'flex', gap: '8px', marginTop: '20px', flexWrap: 'wrap' }}>
            {[
              { label: 'Abonnemang', val: lopande, unit: 'st' },
              { label: 'Att åtgärda', val: attAtgarda, unit: 'st', highlight: true },
              { label: 'Nästa förnyelse', val: nastaFornyelse, unit: '', isStr: true },
            ].map(chip => (
              <div key={chip.label} style={{
                background: chip.highlight && attAtgarda > 0 ? 'rgba(221,82,56,.3)' : 'rgba(255,255,255,.12)',
                border: chip.highlight && attAtgarda > 0 ? '1px solid rgba(221,82,56,.5)' : '1px solid rgba(255,255,255,.18)',
                borderRadius: 'var(--r)',
                padding: '8px 14px',
                display: 'flex', flexDirection: 'column', gap: '1px',
              }}>
                <span style={{ fontSize: '11px', color: 'rgba(255,255,255,.6)', fontWeight: 500 }}>{chip.label}</span>
        ...
        {/* Savings strip — only if there are savings */}
        {sparatIAr > 0 && (
          <motion.div
            initial="hidden" animate="visible" custom={1} variants={fadeUp}
            style={{
              background: 'var(--mint-tint)', border: '1px solid rgba(46,155,114,.15)',
              borderRadius: 'var(--r-lg)', padding: '14px 18px',
              display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', cursor: 'pointer',
            }}
          >
            <div style={{
            ...
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: '14px', fontWeight: 700, color: 'var(--mint-text)' }}>Du har sparat {fmtKr(sparatIAr)} i år</p>
              <p style={{ fontSize: '12px', color: 'var(--mint-deep)', fontWeight: 500 }}>Vakt hittade besparingar åt dig</p>
            </div>
        ...
        {kategorier.length > 0 && (
          ...
          >
            ...
            {/* Bar */}
            <div style={{
              display: 'flex', height: '10px', borderRadius: '99px', overflow: 'hidden', gap: '2px', marginBottom: '14px',
            }}>
              {kategorier.map(k => (
                <div
                  key={k.namn}
                  style={{ width: `${k.andel}%`, background: k.farg, minWidth: 0, transition: 'width .8s ease-out' }}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px 16px' }}>
              {kategorier.map(k => (
                <div key={k.namn} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: k.farg, flexShrink: 0 }} />
                  <span style={{ fontSize: '12px', color: 'var(--text-2)', fontWeight: 500 }}>{k.namn}</span>
                  <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--ink)' }}>{k.andel}%</span>
                </div>
              ))}
            </div>
          </motion.div>
              ...
              {fornyelser.map((f, i) => (
                <div key={f.namn + f.datum} style={{
                  ...
                  <div style={{
                    ...
                    <span style={{ fontSize: '12px', fontWeight: 700, display: 'block', color: f.bradskande ? 'var(--danger)' : 'var(--brand-deep)', lineHeight: 1.2 }}>
                      {f.datum}
                    </span>
                  </div>
                  ...
                  <div style={{ fontSize: '15px', fontWeight: 700, color: f.bradskande ? 'var(--danger)' : 'var(--ink)', flexShrink: 0 }}>
      ...
      <button
        ...
        style={{
          ...
          background: 'var(--brand-deep)', border: 'none', cursor: 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: '0 4px 20px rgba(82,88,232,.4)', zIndex: 40,
...
const cardStyle: React.CSSProperties = {
  ...
  borderRadius: 'var(--r-xl)', padding: '20px', marginBottom: '16px',
